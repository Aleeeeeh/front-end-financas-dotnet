...
import { StatusLancamento } from '../components/typesLancamentos'
...
export default function CadastroLancamento() {
        ...
        if (idLancamento) {
            service.obterPorId(idLancamento)
                .then(response => {
                    ...
                    setTipo(objLancamento.tipoLancamento);
                    setStatus(objLancamento.statusLancamento);
                    setAtualizacao(true)
                })
                .catch(error => {
    ...
    const atualizarLancamento = () => {
        ...
        const objLancamento = {
            ...
            ano: Number(ano),
            tipoLancamento: Number(tipo),
            statusLancamento: Number(status)
        }

        service.atualizar(id, objLancamento)
            .then(response => {
                ...
                <div className="col-md-4">
                    <FormGroup htmlFor="inputStatus" label="Status: *">
                        <input type="text"
                            className="form-control"
                            name="status"
                            value={StatusLancamentoMap[status]}
                            disabled />
                    </FormGroup>
                </div>
            </div>
